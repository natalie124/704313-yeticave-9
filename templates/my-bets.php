    ...
    <table class="rates__list">
        <?php foreach ($bets as $bet): ?>
               ...
               <?= (count_time($bet['dt_end']) < 1 && $bet['win_id'] === $cur_user_id) ? 'rates__item--win' : ''; ?>">
                <td class="rates__info">
                    <div class="rates__img">
                        <img src="<?= htmlspecialchars($bet['img_path']); ?>" width="54" height="40"
                             alt="<?= htmlspecialchars($bet['title']); ?>">
                    </div>
                    <div>
                        <h3 class="rates__title"><a
                                href="lot.php?lot_id=<?= (int)$bet['lot_id'] ?>"><?= htmlspecialchars($bet['title']); ?></a>
                        </h3>
            ...
            <?= (count_time($bet['dt_end']) >= 1 && count_time($bet['dt_end']) < 3600) ? 'timer--finishing' : ''; ?>">
                        ...
                        <?= (count_time($bet['dt_end']) >= 1) ? gmdate("d:H:i",
                            count_time($bet['dt_end'])) : ''; ?></div>
                </td>
