...
<form class="form form--add-lot container <?=$form_invalid ?>" action="add.php" method="post" enctype="multipart/form-data"> <!-- form--invalid -->
  ...
  <div class="form__container-two">
    ...
    <div class="form__item <?=empty($_POST['category']) ? $field_invalid : '' ?>">
  ...
  <div class="form__container-three">
    <div class="form__item form__item--small <?=(empty($_POST['lot-rate']) || !is_numeric($_POST['lot-rate']) || $_POST['lot-rate'] < 1) ? $field_invalid : '' ?>">
      <label for="lot-rate">Начальная цена <sup>*</sup></label>
      <input id="lot-rate" type="text" name="lot-rate" placeholder="0" value="<?=$_POST['lot-rate'] ?? ''; ?>">
      <span class="form__error">Введите начальную цену</span>
    </div>
    <div class="form__item form__item--small <?=(empty($_POST['lot-step']) || !is_numeric($_POST['lot-step']) || $_POST['lot-step'] < 1) ? $field_invalid : '' ?>">
      <label for="lot-step">Шаг ставки <sup>*</sup></label>
      <input id="lot-step" type="text" name="lot-step" placeholder="0" value="<?=$_POST['lot-step'] ?? ''; ?>">
      <span class="form__error">Введите шаг ставки</span>
    </div>
    <div class="form__item <?=(empty($_POST['lot-date']) || !is_date_valid($_POST['lot-date']) || (strtotime($_POST['lot-date']) < (time() + 86399 ))) ? $field_invalid : '' ?>">
